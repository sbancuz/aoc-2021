...
int main(void)
{
    ...
    uint64_t len, grid_count, bytes, winner_number, winner_grid;
    char *strin, *tmp;
    FILE *fin;
    ...
    if ((fin = fopen("04-input.txt", "r")))
    {
            ...
            (tmp = (char *)malloc(sizeof(char) * 10)))
        {
            ...
            if ((grids = (int64_t(*)[GRID_SIZE][GRID_SIZE])malloc(sizeof(*grids) * len)))
            {
                ...
                for (uint64_t i = 0; i < len && !winner_grid; ++i)
                {
                    for (uint64_t j = 0; j < grid_count && !winner_grid; ++j)
                    {
                        write_off(grids[j], called_num[i]);
                        if (check_win(grids[j]))
                        {
                            winner_grid = j;
                            winner_number = called_num[i];
                        }
                    }
                }
                sum = sum_unmarked(grids[winner_grid]);
                printf("%ld * %ld = %ld\n", sum, winner_number, sum * winner_number);
            }
            else
